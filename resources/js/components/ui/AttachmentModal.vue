<template>
    ...
    >
        <div class="fixed inset-0 transition-opacity" aria-hidden="true">
            <div class="absolute inset-0 bg-neutral-focus opacity-75"></div>
        </div>

        <div class="relative bg-base-100 rounded-lg shadow-xl max-w-4xl w-full m-4 p-6" @click.stop>
            <button
                type="button"
                class="absolute top-4 right-4 text-neutral-content hover:text-neutral-focus"
                @click="close"
            >
            ...
            <h2 class="text-2xl font-bold text-base-content mb-6">Manage Attachments</h2>

            <div v-if="error" class="mb-4 p-4 bg-error text-error-content rounded-md">
                {{ error }}
            </div>
            ...
            <div class="mb-6">
                <label
                    for="file-upload"
                    class="flex justify-center items-center px-4 py-6 border-2 border-dashed rounded-md cursor-pointer border-neutral-content hover:border-neutral-focus"
                >
                    <div class="space-y-1 text-center">
                        <svg
                            class="mx-auto h-12 w-12 text-neutral-content"
                            stroke="currentColor"
                            fill="none"
                            viewBox="0 0 48 48"
                            aria-hidden="true"
                        ...
                        <div class="flex text-sm text-neutral-content">
                            <label
                                for="file-upload"
                        ...
                        <p class="text-xs text-neutral-content">PNG, JPG, GIF up to 10MB</p>
                    </div>
                </label>
            </div>

            <div v-if="fileSelected" class="mb-6">
                <div class="flex items-center justify-between p-4 bg-base-200 rounded-md">
            ...
            <div class="space-y-6">
                <div v-if="images.length">
                    <h3 class="text-lg font-medium text-base-content mb-2">Images</h3>
                    <div class="grid grid-cols-3 gap-4">
                        <div v-for="image in images" :key="image.id" class="relative group">
                            ...
                            />
                            <div
                                class="absolute inset-0 bg-neutral-focus bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity duration-150 ease-in-out flex items-center justify-center space-x-2"
                            >
                                <button
                                    type="button"
                                    class="text-white hover:text-primary transition-colors duration-150 ease-in-out p-1 rounded-full bg-neutral-focus"
                                    title="View image"
                                ...
                                <button
                                    type="button"
                                    class="text-white hover:text-error transition-colors duration-150 ease-in-out p-1 rounded-full bg-neutral-focus"
                                    title="Delete image"
